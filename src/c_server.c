...
int main(int argc, char *argv[]){
  ...
  int tcpsockfd, udpsockfd, newsockfd, portno, n;
  unsigned int clilen;
  char buffer[256];
  ...
  while(1){
    ...
    if (tcp == 1){
      ...
      bzero(buffer,256);
      n = recvfrom(udpsockfd, buffer, 255, 0,(struct sockaddr *) &cli_addr, &clilen);
      if (n < 0) error("ERROR reading from socket");
      printf("Message from client: %s\n",buffer);
      sendto(udpsockfd, buffer, n, 0, (struct sockaddr *) &cli_addr, sizeof s);
    }
    else {
      printf("No socket set");
      _exit(0);
    }
    printf("stopped");
  }
